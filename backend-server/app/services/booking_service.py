...
def _normalize_phone(phone: str | None) -> str:
    """Parse to E.164 so "0797911909" and "+254797911909" compare equal
    regardless of which format was typed at booking vs. verification time.
    Falls back to a plain digit comparison if the library can't parse it
    (e.g. a malformed number), rather than raising."""
    if not phone:
        return ""
    try:
...
def cancel_via_link(
    db: Session, appointment_id: int, raw_token: str, phone: str, reason: str | None = None
) -> Appointment:
    appointment = get_appointment_by_link_token(db, appointment_id, raw_token)

    if not phone or _normalize_phone(appointment.patient.phone) != _normalize_phone(phone):
    ...
    appointment.cancelled_reason = reason or "Cancelled by patient via booking link"
    db.commit()
...
def reschedule_booking(db: Session, appointment_id: int, new_slot_time: datetime) -> tuple[Appointment, str]:
    ...
    # Attach the already-loaded doctor/patient so a background email task can
    # read them after this request's session closes (see create_booking).
    ...
    return new_appointment, raw_token


def reschedule_via_link(
    db: Session, appointment_id: int, raw_token: str, phone: str, new_slot_time: datetime
    ...
    return reschedule_booking(db, appointment_id, new_slot_time)
