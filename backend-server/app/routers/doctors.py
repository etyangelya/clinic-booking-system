...
@router.get("/specialists", response_model=list[DoctorResponse])
def list_specialists(db: Session = Depends(get_db)):
    return (
        db.query(Doctor)
        .filter(Doctor.is_active.is_(True), Doctor.is_general.is_(False))
        .order_by(Doctor.specialty, Doctor.name)
        .all()
    )
...
def general_availability(date: date, db: Session = Depends(get_db)):
    ...
    return GeneralAvailabilityResponse(
        date=date,
        available_slots=[
            GeneralAvailabilitySlot(doctor_id=doctor_id, doctor_name=doctor_name, slot_time=slot_time)
            for doctor_id, doctor_name, slot_time in slots
        ],
    )
...
def match_speciality_endpoint(payload: SymptomMatchRequest, db: Session = Depends(get_db)):
    ...
    return SymptomMatchResponse(
        ...
        available_slots=[GeneralAvailabilitySlot(doctor_id=doctor_id,doctor_name=doctor_name, slot_time=slot_time) for doctor_id,doctor_name, slot_time in slots],
    )
