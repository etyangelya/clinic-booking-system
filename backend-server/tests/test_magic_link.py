...
def _book_directly(db_session, doctor, phone="+15551234567", slot=None):
...
def test_link_cancel_with_correct_phone_succeeds(client, db_session, doctor):
    appointment, raw_token = _book_directly(db_session, doctor, phone="+15551234567")

    response = client.patch(
        f"/appointments/{appointment.id}/link-cancel?token={raw_token}&phone=%2B15551234567"
    )
    assert response.status_code == 200
    assert response.json()["status"] == "CANCELLED"


def test_link_cancel_with_differently_formatted_phone_succeeds(client, db_session, doctor):
    appointment, raw_token = _book_directly(db_session, doctor, phone="+1 (555) 123-4567")
...
def test_link_cancel_with_wrong_phone_rejected(client, db_session, doctor):
    appointment, raw_token = _book_directly(db_session, doctor, phone="+15551234567")

    response = client.patch(
        f"/appointments/{appointment.id}/link-cancel?token={raw_token}&phone=%2B15559999999"
    )
...
def test_link_cancel_with_tampered_token_rejected(client, db_session, doctor):
    appointment, _ = _book_directly(db_session, doctor, phone="+15551234567")

    response = client.patch(
        f"/appointments/{appointment.id}/link-cancel?token=tampered-token&phone=%2B15551234567"
    )
    assert response.status_code == 404
