...
def get_general_availability(db: Session, target_date: date_type) -> list[tuple[int, datetime]]:
    general_doctors = db.query(Doctor).filter(Doctor.is_general.is_(True)).filter(Doctor.is_active.is_(True)).all()

    merged: list[tuple[int, datetime]] = []
    for doctor in general_doctors:
        slots = get_available_slots(db, doctor.id, target_date) or []
        merged.extend((doctor.id, slot) for slot in slots)

    return sorted(merged, key=lambda pair: pair[1])
